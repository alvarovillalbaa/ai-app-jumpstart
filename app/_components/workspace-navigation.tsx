...
type Features = { chatEnabled: boolean; accountEnabled: boolean };

function NavigationLinks({ chatEnabled, accountEnabled, close }: Features & { close?: () => void }) {
  const pathname = usePathname();
  return appConfig.navigation.filter(item => item.feature === "core" ||
    (item.feature === "chat" && chatEnabled) || (item.feature === "auth" && accountEnabled)).map(item => {
...
function MobileMenu({ chatEnabled, accountEnabled }: Features) {
  ...
  return <details ref={menu} className="relative">
    ...
    <nav aria-label="Workspace" className="absolute right-0 z-30 mt-2 w-56 rounded-lg border bg-background p-2 shadow-lg">
      <NavigationLinks chatEnabled={chatEnabled} accountEnabled={accountEnabled} close={() => { if (menu.current) menu.current.open = false; }} />
      <div className="mt-2 border-t pt-2"><ThemeSelector /></div>
    </nav>
  </details>;
}

export function WorkspaceNavigation({ chatEnabled, accountEnabled }: Features) {
  return <>
    ...
    <aside className="hidden border-r bg-sidebar text-sidebar-foreground md:flex md:min-h-dvh md:flex-col md:p-4">
      ...
      <nav aria-label="Workspace" className="space-y-1"><NavigationLinks chatEnabled={chatEnabled} accountEnabled={accountEnabled} /></nav>
    ...
    <header className="flex items-center justify-between border-b bg-background px-4 py-3 md:hidden">
      ...
      <MobileMenu chatEnabled={chatEnabled} accountEnabled={accountEnabled} />
    </header>
  </>;
}

export function WorkspaceMenu({ chatEnabled, accountEnabled }: Features) {
  return <MobileMenu chatEnabled={chatEnabled} accountEnabled={accountEnabled} />;
}
