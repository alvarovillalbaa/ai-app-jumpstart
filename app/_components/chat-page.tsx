...
export async function ChatPage({ locator, sessionless = false }: { locator?: string; sessionless?: boolean }) {
  ...
  if (!settings) {
    if (process.env.NODE_ENV === "development") return <AgentChat sessionId={locator} sessionless={sessionless} />;
  ...
  return <OwnedChat key={`${user.id}:${locator ?? "new"}`} settings={settings.auth} userId={user.id} initialOperationId={locator} />;
}
