...
export async function refreshSession(request: NextRequest, forwardedHeaders?: Headers) {
  const next = () => NextResponse.next({ request: { headers: forwardedHeaders ?? request.headers } });
  ...
  const client = createServerClient(settings.url, settings.publishableKey, {
    ...
    cookies: {
      ...
      setAll(values, headers) {
        ...
        if (forwardedHeaders) forwardedHeaders.set("cookie", request.headers.get("cookie") ?? "");
