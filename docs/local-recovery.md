...
With the app stopped, install a verified snapshot into **unused** targets. The app database target must be the configured `SQLITE_PATH`; the Workflow target must be the app root's `.eve/.workflow-data`. Provide `--uploads-dir` with the configured `UPLOAD_LOCAL_ROOT` exactly when the snapshot includes uploads. The command refuses existing targets and old SQLite `-wal` or `-shm` companions; use fresh empty mounts. Eve's default local world selects its Workflow path from the running app root; setting `WORKFLOW_LOCAL_DATA_DIR` alone does not move it.

```sh
npm run backup:local -- --install /PRIVATE_RESTORE_TEST/jumpstart \
  --app-db /FRESH_APP_ROOT/.data/app.sqlite \
  --workflow-dir /FRESH_APP_ROOT/.eve/.workflow-data --stopped
# Add --uploads-dir /FRESH_UPLOAD_ROOT when the snapshot includes uploads.
```

Installation verifies hashes and SQLite integrity again, copies with private permissions, and removes targets it created on an ordinary error. A process crash can leave partial targets; discard those mounts and start with fresh ones. `--stopped` remains an operator assertion, so exclude every writer throughout installation. Restore the matching application build and private environment separately. Start the restored instance against a disposable origin and verify web/data/Eve readiness, owner-isolated REST/CLI/MCP reads, a previously completed conversation and a new owned turn before relying on it. `npm run test:session-runtime` creates a real completed Eve session, snapshots its SQLite application data and local Workflow files after shutdown, installs them in a fresh app root, replays the same session without another model call, denies a foreign owner and completes a new owned turn. Focused tests also cover SQLite WAL capture, private-upload copying, tampering and no-clobber behavior. `npm run test:container` writes the stopped snapshot to a separate Docker volume, verifies and installs it into fresh application and Eve volumes, then boots the production image; the original record and owner-isolated REST/CLI/MCP path survive. That image test does not restore a chat session. A live production backup still needs transaction coordination across stores and deployment-specific recovery acceptance.
...
To recover with Compose, create **new** application and Eve volumes, mount the backup directory read-only and both new volumes in a one-off production-image container, then run `node scripts/backup-local.mjs --install /app/.backup/snapshot-YYYYMMDD --app-db /app/.data/app.sqlite --workflow-dir /app/.eve/.workflow-data --stopped`. Mount local uploads into that container and add their fresh target only when included. Point a replacement service at those new volumes after the install succeeds. Do not attach the old volumes to the replacement service.
