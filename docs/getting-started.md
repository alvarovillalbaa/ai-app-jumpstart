...
For a reproducible records example, put a current account access token or an administrator-issued record token in `APP_API_TOKEN` and run `npm run seed:records` while the application is running. The two fixed records in `scripts/fixtures/records.json` belong to that credential's owner. Run one seed process at a time: sequential reruns leave matching records alone, while an existing title with different content causes an error rather than an overwrite. Record IDs are generated by the selected provider; fixture titles and content are fixed. The seed command reads and writes only through the shared application API, so it works with every data adapter. It refuses a remote `APP_API_URL` unless you explicitly run `npm run seed:records -- --allow-remote`. Seed records are sample private data, not accounts or production data.

After changing SQL migrations, start local Supabase, apply pending repository migrations, then run `npm run db:types` to regenerate `lib/data/supabase.generated.ts`. `npm run db:types:check` verifies the committed types match that local schema. Both commands fail if the local migration ledger is behind; they do not touch a hosted project. The generated types describe the Supabase/PostgREST schema, while SQLite and Convex retain their own adapter contracts.
...
Before a managed Vercel release, set `AI_CHAT_ENABLED` explicitly and run `npm run check:managed-config` against its intended private runtime values; add `-- --require-chat` when account chat must be enabled. For enabled chat, also run `npm run check:budget-policy` and review its quoted amount against current provider prices and realistic token limits. These are offline checks, not substitutes for migration review or hosted acceptance.
