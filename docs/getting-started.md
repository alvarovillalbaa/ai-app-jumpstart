...
To start over without losing data, stop and restart the local Supabase services. For a truly fresh local dataset, create a separate disposable project/clone and migrate that database; never treat a reset of a hosted project as routine setup. Release migrations require a reviewed backup and an operator who knows which environment is targeted.
