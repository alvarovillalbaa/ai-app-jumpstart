...
Private upload metadata is available through owner-scoped REST, CLI and MCP. An optional self-hosted `UPLOAD_DOWNLOAD_POLICY=scan-on-read` enables `GET /api/v1/uploads/UPLOAD_UUID/download`, CLI `uploads download UPLOAD_UUID OUTPUT_FILE` and a browser download button. Each request requires the distinct `uploads:download` authorization (granted to signed-in users), checks stored bytes against catalog metadata and obtains a fresh clean private-scanner verdict before returning an attachment. The CLI writes a private no-clobber file. MCP remains metadata-only; no tool returns quarantined bytes. The default policy, managed Vercel path and unavailable scanner deny downloads. See [private uploads](uploads.md).
