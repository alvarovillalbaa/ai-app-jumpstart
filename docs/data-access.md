...
npm run app -- export source-events OPERATION_UUID .data/conversation-source.ndjson
npm run app -- export verify .data/application-export.ndjson
```

The command reads existing authenticated REST pages and writes newline-delimited JSON. For records and application exports, the first line is a versioned `manifest` (`ai-app-jumpstart-visible-data-v5`), each following line has a `type` and `value`, and the final `end` line has counts and `contentSha256`, the SHA-256 digest of every preceding line including its newline. Application mode writes one `account_profile` line before owner-scoped records. It pages until each available collection ends, including archived conversations and each conversation's projections. The upload section records all active catalog entries in their current `pending`, `quarantined`, `clean`, `rejected` or `deleting` state, including their last scan decisions when present, followed by one `upload_usage` line. Paged `budget_reservation` lines include each admission timestamp, policy, estimate, status and nullable actual cost; `null` means cost is unknown even after settlement. Paged `budget_correction` lines include the affected operation, previous and corrected cost, and correction timestamp; operator notes and evidence references are omitted. The output is mode `0600` and is published only after all reads succeed; it never replaces an existing file. Keep the file private and outside version control. Each page is a live read, so concurrent changes may appear or be missed; a nonadvancing cursor or more than 10,000 pages for one collection fails without publishing a partial file. A failed or expired token also fails the export.

`source-events` exports one active owned conversation's selected Eve stream events in absolute source order. It requires a current registered-user token, enabled account chat and a reachable retained runtime stream. Its manifest is `ai-app-jumpstart-source-events-v1`; each `source_event` retains `sourceIndex`, and the final `end` line records the count, next index, `complete:true` and the same content digest. The exporter advances across pages containing no selected events, fails if a page or the runtime fails, and never publishes a partial file. It captures the tail observed during its finite reads; later events may be absent. These events include interrupted attempts and are neither canonical model history nor a complete transcript. This separate export does not change the application-visible v5 format or its exclusions.
...
Private upload metadata is available through owner-scoped REST, CLI and MCP. Optional `UPLOAD_DOWNLOAD_POLICY=scan-on-read` enables `GET /api/v1/uploads/UPLOAD_UUID/download`, CLI `uploads download UPLOAD_UUID OUTPUT_FILE` and a browser download button when a private Unix-socket or authenticated HTTPS scanner is configured. Each request requires the distinct `uploads:download` authorization (granted to signed-in users), checks stored bytes against catalog metadata and obtains a fresh clean verdict before returning an attachment. The CLI writes a private no-clobber file. Optional `UPLOAD_DOWNLOAD_SIGNING_JSON` enables `POST /api/v1/uploads/UPLOAD_UUID/download-link` with `{}`, CLI `uploads link UPLOAD_UUID` and MCP `uploads_download_link`. These return a relative application URL that expires in 60 seconds and still requires the current owner credential and a fresh scan when redeemed; see [link/key-rotation semantics](uploads.md#short-lived-owner-download-links). `POST /api/v1/uploads/UPLOAD_UUID/scan` with `{}`, CLI `uploads scan UPLOAD_UUID` and MCP `uploads_scan` run the same authorized checks and persist a metadata-only decision. A clean decision is historical: every download still needs a fresh scan. Rejected IDs stay rejected even if the scanner later returns clean; replacement needs a new upload ID. MCP returns metadata only; no tool returns private upload bytes. The default policy and unavailable scanner deny downloads. The managed path requires a separately deployed [remote scanner](remote-upload-scanner.md); its hosted behavior remains unverified. See [private uploads](uploads.md).
