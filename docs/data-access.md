...
The current [AI usage view](usage-budgets.md) is readable at `/usage`, `GET /api/v1/usage`, CLI `usage`, MCP `usage_get` and `usage:///current`. These all use the verified account owner; record API keys do not grant budget access. Values are application reservations and settlements, including conservative unknown-cost estimates.
