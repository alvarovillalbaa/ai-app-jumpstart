...
`GET /api/v1/usage/reservations?limit=50&cursor=...`, CLI `npm run app -- usage reservations [--limit N] [--cursor CURSOR]`, and MCP `usage_reservations` page the verified owner's historical reservation rows, including admission time, policy ID, estimate, current status and nullable actual cost. A settled row with `actualMicros: null` has an unknown cost and charges its estimate in the daily view. The timestamp/operation cursor handles multiple reservations admitted in the same millisecond. The read never exposes request hashes or other owners' entries.

`GET /api/v1/usage/corrections?limit=50&cursor=...`, CLI `npm run app -- usage corrections [--limit N] [--cursor CURSOR]`, and MCP `usage_corrections` page the same owner's cost corrections across operations, without the operator actor, reason or evidence reference. The timestamp/correction-ID cursor avoids silently truncating an operation after 100 corrections, unlike the separate operator preview command. Both account history views are live paged reads, so concurrent changes may appear or be missed. The [application-visible export](data-access.md#export-visible-application-data) includes these selected rows; model-attempt IDs, operator correction notes/evidence and historical daily aggregates still require separate export coverage.
...
| `costBasis.sourceUrl` / `reviewedAt` | HTTPS pricing source and date of operator review |
| `costBasis.models` | Exactly one entry per allowed model ID, with `maxInputTokens`, `maxOutputTokens`, `inputMicrosPerMillion` and `outputMicrosPerMillion` |
| `costBasis.maxOtherMicros` | Additional allowance for paid tools or other costs not included in model-token prices |

Any enabled account chat requires `costBasis`. The validator rounds each model's input and output quote up to whole micro-USD, takes the most expensive allowed model, multiplies it by `maxModelCalls`, adds `maxOtherMicros`, and rejects `estimateMicros` below that result or above the daily allowance. It uses integer arithmetic even when intermediate products exceed JavaScript's safe integer range. The same parse runs in Next's account-chat configuration and Eve's runtime; the managed-host preflight exercises it without making a provider call. Test fixtures use attributed synthetic prices.

Run `npm run check:budget-policy` with the private policy in `.env.local` before a release. It prints the calculated micro-USD quote, reservation, model IDs, review date and pricing-source host, or field names that need correction. It makes no provider call and prints no policy JSON or keys. Update the policy ID, source and review date whenever prices or token assumptions change; rehearse the actual production model separately.

There is no default price or allowance. Use current provider and routing prices for the exact model ID and document the reviewed token assumptions. **The quoted token counts are operator assumptions, not per-call limits enforced by this validator.** Eve's session token limits are checked after a call and permit a user continuation; they cannot turn this arithmetic check into a hard invoice ceiling. Input growth, provider retries, compaction, changing prices and paid tools can exceed a reservation; unknown or higher actual cost remains visible and may require reconciliation. Review the model's actual context/output limits and production-model behavior before enabling paid chat. The root agent disables Eve's default tool set and exposes the authored tools; its model selection remains unchanged. Paid tools or additional agents need their own cost admission before being enabled. Development sessions using Eve's local authenticator do not enter this application-owned budget path.
...
The real Eve fixture verifies owned follow-ups, manual compaction settlement, daily-limit denial, tool-loop call caps and a queued forged approval that does not produce another model call or enlarge the budget. It uses a fresh temporary app/queue namespace and the actual production hook. The account browser contract checks the usage screen and REST/CLI/MCP parity with a real authenticated user after quota exhaustion, then checks another account sees zero. It also proves budget-only cancellation in a disposable ledger. Cost-basis tests cover exact model coverage, round-up arithmetic, underquotes, missing reviews and overflow. Remaining work includes populating and verifying actual provider prices/token assumptions, disputed-start reconciliation beyond owner cancellation and deployed acceptance. These local checks do not certify every optional tool, agent, callback or provider configuration.
