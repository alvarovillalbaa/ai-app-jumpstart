...
`GET /api/v1/usage/reservations?limit=50&cursor=...`, CLI `npm run app -- usage reservations [--limit N] [--cursor CURSOR]`, and MCP `usage_reservations` page the verified owner's historical reservation rows, including admission time, policy ID, estimate, current status and nullable actual cost. A settled row with `actualMicros: null` has an unknown cost and charges its estimate in the daily view. The timestamp/operation cursor handles multiple reservations admitted in the same millisecond. The read never exposes request hashes or other owners' entries. It is a live paged view, so concurrent changes may appear or be missed. The [application-visible export](data-access.md#export-visible-application-data) includes these rows; model-attempt IDs, correction audit entries and historical daily aggregates still require separate export coverage.
