...
import { libpqEnvironment } from "../../scripts/backup-postgres.mjs";
...
it("passes PostgreSQL URL credentials through libpq environment without inheriting another target", () => {
  const result = libpqEnvironment(
    ...
    { NODE_ENV: "test", PGHOST: "wrong.example", PGPASSWORD: "old", PGSERVICE: "production", DATABASE_URL: "old-secret", OTHER_SETTING: "kept" },
  );
