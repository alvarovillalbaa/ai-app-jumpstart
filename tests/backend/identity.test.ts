...
it("uses the verified identity, excludes editable metadata, and does not cache identity responses", async () => {
  ...
  expect(await authenticate(request(), settings)).toEqual({ tenant: "supabase:https://identity.example", subject: user.id, scopes: ["records:read", "records:write"], credentialType: "user" });
